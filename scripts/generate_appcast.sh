#!/usr/bin/env bash
#
# 根据本地归档的 ConnectMate DMG 生成 Sparkle appcast.xml，并将下载地址改写为 GitHub Releases 资源地址。
#
...
DEFAULT_ACCOUNT="cn.vanjay.ConnectMate.sparkle"
...
detect_repo() {
    ...
    repo_url="$(read_plist_string "ConnectMateGitHubURL")"
    if [[ -n "$repo_url" ]]; then
...
TMP_DIR="$(mktemp -d "${TMPDIR:-/tmp}/connectmate-appcast.XXXXXX")"
...
title_node.text = "ConnectMate Updates"
...
description_node.text = "ConnectMate release feed."
